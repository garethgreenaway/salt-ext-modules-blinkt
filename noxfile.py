...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/blinkt/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/blinkt/*")
            ...
            #    "--include=src/saltext/blinkt/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/blinkt/config/schemas",
    )
